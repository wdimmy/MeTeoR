...
class BuchiAutomata:
    ...
    def build_prior(self, must_literals):
        ...
        for ruler_interval in initial_window_ruler_intervals:
            # add facts with infinity points
            for predicate in self.D:
                for entity in self.D[predicate]:
                    for interval in self.D[predicate][entity]:
                        if interval.right_value is decimal.Decimal('Infinity'):
                            if interval.left_vale <= ruler_interval.right_value:
                                 literal1 = Literal(Atom(predicate),
                                                   [Operator("Boxmplus", Interval(0, Decimal("inf"), False, True))])
                                 literal2 = Literal(Atom(predicate),
                                                   [Operator("Boxmplus", Interval(0, Decimal("inf"), True, True))])
                                 self.window_must_include_static[ruler_interval].append(literal1)
                                 self.window_must_include_static[ruler_interval].append(literal2)
                        if interval.left_value is decimal.Decimal('-Infinity'):
                            if interval.right_vale >= ruler_interval.left_value:
                                literal1 = Literal(Atom(predicate),
        ...
        program_variables = set() # ground the metric atoms in the rules with constants appearing in D and Pi
            ...
            if len(T) == 0:
                ...
                for ruler_interval in ruler_intervals:
                    if interval_intesection_intervallist(ruler_interval, T) and literal not in \
                        ...
                        ruler_intervals_literals[ruler_interval].add(literal)
