...
def consistency(D_1, program, F=None):
    ...
    if F is None:
        ...
        if "Bottom" in D and len(D["Bottom"]) !=0:
            return False
        ...
        materialize_pre(D, rules=relevant_rules, non_recursive_predicates=non_recursive_predicates, D_index=D_index, must_literals=must_literals)
        d_atoms, d_unbounded_literals = extract_dataset(D, involved_predicates, automata_predicates)
        points, x = get_dataset_points_x(D)
        z, gcd = get_gcd(relevant_rules)

        left_dict, right_dict = construct_left_right_pattern(points, gcd)
        materialize_limit(D, relevant_rules, D_index, limit=Interval(-2*z-2*x, 2*x+2*z, False, False), must_literals=must_literals)
